...
export const autoSwap_Buy_thread = async (web3, database, bot) => {
    ...
    for (const user of users) {
        ...
        // console.log("buy dist = ", session.dist_finished)
        
        if (!session || !token_address || session.dist_finished == 0) {
            continue
        }
        ...
        if (!session.swap_start || !GetBuyDurationTimeOut(session.swap_end_time))
        {
    ...
    setTimeout(() => {
        autoSwap_Buy_thread(web3, database, bot)
    }
    , INTERVAL) 
